	...
	if err1 != nil {
	   errlog.Println("OShotMMS Table 조회 중 중 오류 발생", err1)
		...
		if !s.EqualFold(msgcnt.String, "0") {	
			...
			db.Exec("insert into " + SMSTable + " SELECT * FROM SMS_MSG_G WHERE TR_SENDSTAT='2' AND TR_RSLTDATE is not null and telecom = 'ETC'")
			db.Exec("delete FROM SMS_MSG_G WHERE TR_SENDSTAT='2' AND TR_RSLTDATE is not null and telecom = 'ETC'")
		}
	}

	var groupQuery = "select a.TR_ETC10 as mst_id, b.mst_mem_id as mem_id, (select mem_userid from cb_member cm where cm.mem_id = b.mst_mem_id) AS mem_userid, b.mst_sent_voucher from " + SMSTable + " a inner join cb_wt_msg_sent b on a.remark4 = b.mst_id where a.TR_SENDSTAT = '2' and a.TR_ETC8 = 'Y' "
