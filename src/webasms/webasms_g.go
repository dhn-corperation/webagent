...
func Process_g(ctx context.Context) {
	config.Stdlog.Println("나노 SMS(저가망) (WEB A) - 결과 처리 프로세스 시작")
	var wg sync.WaitGroup
	for {
		select {
			case <- ctx.Done():
				config.Stdlog.Println("나노 SMS(저가망) (WEB A) - process가 15초 후에 종료")
			    time.Sleep(15 * time.Second)
			    config.Stdlog.Println("나노 SMS(저가망) (WEB A) - process 종료 완료")
				return
			default:
				wg.Add(1)
				go smsProcess_g(&wg)
...
func smsProcess_g(wg *sync.WaitGroup) {
	...
	defer func() {
		if r := recover(); r != nil {
			config.Stdlog.Println("나노 SMS(저가망) (WEB A) - panic 발생 원인 : ", r)
			if err, ok := r.(error); ok {
				if s.Contains(err.Error(), "connection refused") {
					for {
						config.Stdlog.Println("나노 SMS(저가망) (WEB A) - send ping to DB")
	...
	if err1 != nil {
	   errlog.Println("나노 SMS(저가망) (WEB A) - 조회 중 중 오류 발생 err :", err1)
	   panic(err1)
	} else {		
	...
	if cnterr != nil && cnterr != sql.ErrNoRows {
		errlog.Println("나노 SMS(저가망) (WEB A) -", SMSTable, "Table - select error : " + cnterr.Error())
		if s.Index(cnterr.Error(), "1146") > 0 {
			db.Exec("Create Table IF NOT EXISTS " + SMSTable + " like SMS_LOG_G")
			errlog.Println("나노 SMS(저가망) (WEB A) -", SMSTable + " 생성 !!")
		}
		time.Sleep(10 * time.Second)
		return
	} else {
	...
	if err != nil {
		errcode := err.Error()
		errlog.Println("나노 SMS(저가망) (WEB A) - 조회 중 오류 발생", groupQuery, errcode)
	...
	if isProc {
		...
		for groupRows.Next() {
			...
			if err != nil {
				errlog.Println("나노 SMS(저가망) (WEB A) - 조회 중 오류 발생")
			...
			tx, err := db.Begin()
			if err != nil {
				errlog.Println("나노 SMS(저가망) (WEB A) - 트랜잭션 시작 중 오류 발생")
				// errlog.Fatal(err)
			}
			...
			for rows.Next() {
				...
				if len(upmsgids) >= 1000 {
					...
					if err1 != nil {
						errlog.Println("나노 SMS(저가망) (WEB A) -", SMSTable + " Table Update 처리 중 오류 발생 ")
					}
				...
				if len(amtsStrs) >= 1000 {
					...
					if err != nil {
						errlog.Println("나노 SMS(저가망) (WEB A) - AMT Table Insert 처리 중 오류 발생 " + err.Error())
					}
			...
			tx.Commit()
			stdlog.Printf("나노 SMS(저가망) (WEB A) - ( %s ) WEB(A) 저가망 SMS 처리 - %s : %d \n", startTime, sent_key.String, smscnt)
		}
	}
}
