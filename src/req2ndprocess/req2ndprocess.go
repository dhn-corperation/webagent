...
func resProcess(wg *sync.WaitGroup) {
	...
	if !s.EqualFold(cnt.String, "0") {
		...
		resp, _ := db.Exec("update " + conf.REQTABLE2 + " as t2 set remark3 = 'P' where t2.remark3 = 'Y' and not exists (select 1 from " + conf.REQTABLE1 + " as t1 where t1.msgid = replace(t2.msgid,'AT', '') )")
